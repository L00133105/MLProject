...
var fs = require('fs');
var array = fs.readFileSync('sitesTravel.txt').toString().split("\n");
for(i in array) 
{
    var url = array[i];
    request(url, (error, response, html) => 
    {
        ...
        {
            var wstream = fs.createWriteStream('DataTravel.txt');
...
const OUTPUT_FILE = 'wordsTravel.json'
var testString = fs.readFileSync('DataTravel.txt').toString();
...
let countArray=[];
        ...
        const autoTrain = require('./wordsTravel.json');
        trainedPath = 'wordsTravel.json'
        let addWords=[];
                autoTrain.forEach((item) => {
                  if (item.count >= 30)
                  addWords.push({"text":item.word,"category":"travel"});
                });
