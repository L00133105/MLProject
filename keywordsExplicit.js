...
var fs = require('fs');
var array = fs.readFileSync('sitesExplicit.txt').toString().split("\n");
for(i in array) {
    var url = array[i];
    request(url, (error, response, html) => {
        if(!error && response.statusCode==200)
        {
            var wstream = fs.createWriteStream('DataExplicit.txt');
...
const OUTPUT_FILE = 'wordsExplicit.json'
var testString = fs.readFileSync('DataExplicit.txt').toString();
...
let countArray=[];
        ...
        const autoTrain = require('./wordsExplicit.json');
        trainedPath = 'wordsExplicit.json'
        let addWords=[];
                autoTrain.forEach((item) => {
                  if (item.count >= 30)
                  addWords.push({"text":item.word,"category":"explicit"});
                });
