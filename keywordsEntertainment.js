...
var fs = require('fs');
var array = fs.readFileSync('sitesEntertainment.txt').toString().split("\n");
for(i in array) 
{
    var url = array[i];
    request(url, (error, response, html) => 
    {
        ...
        {
            var wstream = fs.createWriteStream('DataEntertainment.txt');
...
const OUTPUT_FILE = 'wordsEntertainment.json'
var testString = fs.readFileSync('DataEntertainment.txt').toString();
        ...
        function writeCount(json, filePath) {
          ...
          let addWords=[];
                  autoTrain.forEach((item) => {
                    if (item.count >= 30)
                    addWords.push({"item":item.word,"category":"entertainment"});
                  });
