...
for(i in array) 
{
    ...
    request(url, (error, response, html) => 
    {
        ...
        {
            var wstream = fs.createWriteStream('DataSports.txt');
...
const OUTPUT_FILE = 'wordsSport.json'
...
let countArray=[];
        ...
        const autoTrain = require('./wordsSport.json');
        trainedPath = 'wordsSport.json'
        let addWords=[];
                autoTrain.forEach((item) => {
                  if (item.count >= 30)
                  addWords.push({"text":item.word,"category":"sport"});
                });
